...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (existingDataset) {
      return NextResponse.json({ message: 'A dataset with this name already exists' }, { status: 400 })
    }

    const userDatasets = await db.collection("datasets").find({ userId }).toArray()
    if (userDatasets.length >= 3) {
      return NextResponse.json({ message: 'Maximum number of datasets reached' }, { status: 400 })
    }
    ...
    return NextResponse.json({ message: 'Failed to create dataset' }, { status: 500 })
  }
}